console.log('Версия 3.2 Release');
...
function defaultTab1() {
    ...
    styleElement.innerHTML = '#tab1,#tab1>div>.vkuiTabbarItem__icon{color:var(--vkenhancer--chosen_tab)!important}#Photo,#NFT,#Emoji,#postR,#idName,#GroupsRecent,#ScrollBar{display:flex!important;}#CustomAccentChoose,#CustomTextChoose,#CustomTextChoose1,[aria-customfont="true"],[aria-custombg="true"],[aria-customlogo="true"],#SliderBlock{display:block!important}';
...
function defaultTab3() {
    ...
    styleElement.innerHTML = '#tab3,#tab3>div>.vkuiTabbarItem__icon{color:var(--vkenhancer--chosen_tab)!important}#AwayR,#PollsRes,#SaveSettings,#ClearSettings,#LoadSettings,#ReloadVKE,#SecretOldDesign,#HiderL,#CallsM{display:flex!important;}[aria-id="true"]{display:block!important}';
    chrome.storage.local.set({
        defaultTab: "3",
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    styleElementVersion.innerHTML = "#version::after{content:'Версия 3.2 Release'} #version1::after{content:'Не обновляется расширение? Нажмите CTRL+M в любом месте браузера и расширение перезагрузится, при этом, обновившись до новейшей версии'}";
