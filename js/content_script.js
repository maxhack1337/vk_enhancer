...
function _injectScript(scriptSrc) {
...
chrome.storage.local.get(["newDesignState"], function(items) {
if(items.newDesignState) {
window.postMessage({ action: "vkNewDesign" }, "*")
_injectScript("js/vkm_reforged_in_vkcom.js");
}
else {
window.postMessage({ action: "vkNewDesignOff" }, "*")
_injectScript("js/legacy_in_vkcom.js");
}
});

window.addEventListener("vkNav", (e) => {
    chrome.storage.local.get(["newDesignState"], async ({ newDesignState }) => {
        if (newDesignState) {
            console.log('Новый дизайн мессенджера');
            window.postMessage({ action: "vkNewDesign" }, "*")
...
function create(name, styles, options) {
    ...
    styles && (Object.keys(styles).forEach(style_ => tmp.style[style_] = styles[style_]))
    options && (Object.keys(options).forEach(option_ => tmp[option_] = options[option_]))
    return tmp;
}
...
function createReloadButton() {
	...
	reloadButton.setAttribute('onmouseover', "showTooltip(this, { text: 'Перезагрузить функции VK Enhancer', black: true, shift: [4, 5] });");
    reloadButton.addEventListener('click', (event) => {
        chrome.storage.local.get(["cameraPhotoState", "addstickerState", "customHotbar", "muteCallsState", "altSBState", "recentGroupsState", "emojiStatusState", "sliderValue", "checkboxStateAva", "checkboxState", "checkboxState1", "secretFuncState", "postReactionsState", "hiderState", "customAccent", "colorPicker", "colorPickerText", "customLogo", "customBg", "customFont"], ({
            checkboxState,
            checkboxState1,
            postReactionsState,
            secretFuncState,
            ...
            addstickerState,
            cameraPhotoState
        }) => applyStyles(checkboxState, checkboxState1, postReactionsState, secretFuncState, hiderState, customAccent, colorPicker, colorPickerText, customLogo, customBg, customFont, checkboxStateAva, sliderValue, emojiStatusState, recentGroupsState, altSBState, muteCallsState, customHotbar, addstickerState, cameraPhotoState));
...
function HotBarAppear(cHotBarValue) {
    ...
    if (!existingHotbar && cHotBarValue.length > 0) {
        ...
        const tooltip = create("span", {
            ...
            fontFamily: 'var(--palette-vk-font,-apple-system,BlinkMacSystemFont,"Roboto","Helvetica Neue",Geneva,"Noto Sans Armenian","Noto Sans Bengali","Noto Sans Cherokee","Noto Sans Devanagari","Noto Sans Ethiopic","Noto Sans Georgian","Noto Sans Hebrew","Noto Sans Kannada","Noto Sans Khmer","Noto Sans Lao","Noto Sans Osmanya","Noto Sans Tamil","Noto Sans Telugu","Noto Sans Thai",arial,Tahoma,verdana,sans-serif)'
        ...
        const imgElementReboot = create("img", { scale: '0.75' }, {
            className: 'emoji',
...
function handleWlPostMutation(mutationsList, observer) {
    for (const mutation of mutationsList) {
        ...
        if (!document.getElementsByClassName('VKCOMMessenger__skeletonContent').length > 0 && mutation.type === 'childList' && mutation.addedNodes.length > 0 && window.location.href.includes("im")) {
			...
			const wlPostElement1 = document.getElementsByClassName('Composer__input');
            ...
            if (wlPostElement || wlPostElement1) {
...
function applyStyles(isOldAccentChecked, isMsgReactionsChecked, isPostReactionsChecked, isSecretChecked, isHiderChecked, cAccentValue, cColorValue, cTextValue, cLogoValue, cBgValue, cFontValue, isNameAva, sliderValueCount, emojiStatusChecked, recentGroupsChecked, altSBChecked, muteCallsChecked, cHotBarValue, addStickerChecked, cameraPhotoChecked, hideButtonChecked, newDesignChecked, integrationMediaChecked) {
	...
	if(integrationMediaChecked) {
		console.log("New int");
		window.postMessage({ action: "integrationMedia", value: integrationMediaChecked }, "*")
	}
	else {
		console.log("Old int");
		window.postMessage({ action: "integrationMedia", value: integrationMediaChecked }, "*")
...
function applySavedStyles() {
    chrome.storage.local.get(["integrationMediaState","newDesignState", "hideButtonState", "cameraPhotoState", "addstickerState", "customHotbar", "muteCallsState", "altSBState", "recentGroupsState", "emojiStatusState", "sliderValue", "checkboxStateAva", "checkboxState", "checkboxState1", "secretFuncState", "postReactionsState", "hiderState", "customAccent", "colorPicker", "colorPickerText", "customLogo", "customBg", "customFont"], function (items) {
...
function runStickerAdder() {
    ...
    stickerLink.addEventListener('click', function () {
        ...
        const title = create("div", {}, { className: 'box_title', textContent: 'Прикрепление стикера' });
        const textBox = create("input", { width: '100%', marginBottom: '10px' }, { type: 'text', placeholder: 'Введите ID стикера', pattern: 'd*' })
        ...
        const closeButton = create('div', { "position": "absolute", "top": "10px", "right": "10px" }, {});
        closeButton.addEventListener('click', function () {
            overlay.remove();
        });
        const closeIcon = create('svg', { "width": "20px", "height": "20px", "display": "block" }, {})
